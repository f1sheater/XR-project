    ...
    void Update()
    {
        ...
        if (grabbing)
        {
            ...
            if (grabbedObject)
            {
                ...
                Quaternion deltaRotation = transform.rotation * Quaternion.Inverse(rotationSave);
                grabbedObject.rotation = deltaRotation * grabbedObject.rotation;

                Vector3 rotationVector = grabbedObject.position - transform.position;
                rotationVector = deltaRotation * rotationVector - rotationVector;
                
                Vector3 deltaPosition = transform.position - positionSave + rotationVector;
